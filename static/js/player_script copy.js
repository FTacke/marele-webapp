...
function pad(number) {
  ...
  function loadTranscription(transcriptionFile, transcriptionContainer) {
    fetch(transcriptionFile)
        ...
        .then(transcriptionData => {
            ...
            segments.forEach((segment, index) => {
                ...
                speakerElement.addEventListener('click', function() {
                  // Extrahiere die Startzeit des ersten Wortes im Segment
                  const firstWordStartTime = words[0].start;
                
                  // Extrahiere die Endzeit des letzten Wortes im Segment
                  const lastWordEndTime = words[words.length - 1].end;
      
                  // Starte das Abspielen des Audios vom Startzeitpunkt bis zum Endzeitpunkt
                  playVisualAudio(firstWordStartTime, lastWordEndTime, true);
                
                  // Gib die Start- und Endzeit in der Konsole aus
                  console.log(`Speaker: ${speakerName} Start: ${firstWordStartTime} End: ${lastWordEndTime}`);
                });

                // Event-Listener für den Sprechernamen, um Audio abzuspielen und den Button zu aktualisieren
                speakerElement.addEventListener('click', function () {
                  // Überprüfen Sie, ob der Audioplayer existiert
                  if (audioPlayer) {
                    // Überwachen Sie den Wiedergabestatus
                    if (audioPlayer.paused || audioPlayer.ended) {
                      // Wenn der Player pausiert ist oder beendet wurde, starten Sie die Wiedergabe
                      audioPlayer.play();
                    } else {
                      // Wenn der Player gerade spielt, pausieren Sie die Wiedergabe
                      audioPlayer.pause();
                    }

                    // Aktualisieren Sie den Play/Pause-Button
                ...
                words.forEach((word, index) => {
                    ...
                    // Überprüfen, ob das Word das Attribut "type" mit dem Wert "list" hat
                    if (word.type === 'list') {
                        const wordListDiv = document.createElement('div');
                        wordListDiv.id = 'wordlist';
                        wordListDiv.appendChild(wordElement);
                        transcriptionElement.appendChild(wordListDiv);
                    } else if (word.type === 'text') {
                        const wordTextDiv = document.createElement('div');
                        wordTextDiv.id = 'wordtext';
                        wordTextDiv.appendChild(wordElement);
                        transcriptionElement.appendChild(wordTextDiv);
                    } else {
              ...
              if (event.ctrlKey) {
                ...
                // Berechnung des Startzeitpunkts, einschließlich 0,25 Sekunden vor dem geklickten Wort
                const startPrev = index > 0 ? Math.max(0, parseFloat(words[index - 0].start) - 0.25) : 0;
            
                // Berechnung des Endzeitpunkts, einschließlich 0,25 Sekunden nach dem geklickten Wort
                const endNext = index < words.length - 1 ? Math.min(parseFloat(words[index + 0].end) + 0.25, parseFloat(words[words.length - 1].end)) : parseFloat(words[words.length - 1].end);
            
            

                playVisualAudio(startPrev, endNext, true);

                // Setze die Start- und Endzeitpunkte im Dataset des Wortes, nur wenn Strg nicht gedrückt ist
                if (!event.ctrlKey) {
            ...
            document.addEventListener('keydown', function(event) {
              // Überprüfen Sie, ob die gedrückte Taste die Leertaste ist und die Strg-Taste gedrückt wird
              if (event.code === 'Space' && event.ctrlKey && !isCtrlSpacePressed) {
                // Markieren, dass die Tasten gedrückt wurden, um wiederholtes Auslösen zu verhindern
                isCtrlSpacePressed = true;
            
                // Wenn der Visual-Audio-Player existiert
                if (visualAudioPlayer) {
                  // Überwachen Sie den Wiedergabestatus
                  if (visualAudioPlayer.paused || visualAudioPlayer.ended) {
                    // Wenn der Player pausiert ist oder beendet wurde, starten Sie die Wiedergabe
                  ...
                  // Verhindern Sie, dass der Browser das Standardverhalten für die Leertaste ausführt (z.B. scrollen der Seite)
                  event.preventDefault();
                  
                  // Aktualisieren Sie den Play/Pause-Button nach dem Drücken von Strg + Leertaste
                  updatePlayPauseButton();
                }
              }
            });
            ...
            document.addEventListener('keyup', function(event) {
              // Markiere, dass die Tasten nicht mehr gedrückt werden
        ...
        // Assuming the audio file is in the same domain, otherwise, you might need to handle CORS.
  fetch(audioFile, { headers: { 'Range': 'bytes=0-99999' } }) // Beispielbereich
    .then(response => {
        if (!response.ok && response.status !== 206) {
            throw new Error('Fehler beim Laden des Audios: Antwort nicht OK');
        }
        return response.arrayBuffer();
    })
    .then(buffer => {
        // Hier haben Sie den ArrayBuffer der ersten 100.000 Bytes der Audiodatei
        audioBuffer = buffer;
        // Hier können Sie weitere Aktionen mit dem audioBuffer durchführen
    })
